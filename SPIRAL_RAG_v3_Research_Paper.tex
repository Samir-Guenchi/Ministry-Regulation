...
\title{
  \vspace{-1cm}
  {\color{primaryblue}\Large\bfseries SPIRAL-RAG v3.1: Five Architectural Innovations with Peer-Review Revisions}\\[6pt]
  ...
  {\small Self-reflective Parallel Iterative Retrieval with Adaptive Language — v3.1 Revision}
}
...
\begin{abstract}
We present \textbf{SPIRAL-RAG v3.1}, an extended Retrieval-Augmented Generation (RAG)
...
the peer-reviewed v2 architecture, v3 introduces five novel contributions, and v3.1
incorporates twelve targeted revisions addressing peer-review concerns
(W1--W12) including dense-semantic temporal supersession detection, SetFit-based
authority classification, multi-label intent routing, and SSE streaming.
...
\innov{3}{Temporal Supersession Detector — Dense Cosine Similarity + LLM confirmation
...
\For{each pair $(e_i, e_j) \in E \times E$, $i < j$}
  ...
  \State $S \gets \text{CosineSim}(\mathbf{v}(e_i), \mathbf{v}(e_j))$
  \If{$S < 0.72$} \textbf{continue} \EndIf
  ...
  \EndIf
  \State $\text{alerts.append}(\{e_{\text{new}}, e_{\text{old}}, S, \text{confirmed}, v\})$
...
\paragraph{Dense cosine similarity.} Following reviewer feedback (W3), v3.1 replaces
token-set Jaccard with \emph{dense cosine similarity} over the Gemini
\texttt{text-embedding-004} vectors already computed by the \texttt{DenseRetriever}:
\begin{equation}
S(c_i, c_j) = \frac{\mathbf{v}(c_i) \cdot \mathbf{v}(c_j)}{\|\mathbf{v}(c_i)\|\,\|\mathbf{v}(c_j)|}
\label{eq:cosine_sim}
\end{equation}
where $\mathbf{v}(c) \in \mathbb{R}^{768}$ is the pre-computed dense embedding of chunk $c$.
Dense cosine similarity captures semantic overlap even when a newer decree employs
entirely new vocabulary to replace an older circular — the key failure mode of
lexical Jaccard that reviewers identified. The threshold $S \geq 0.72$ is calibrated
on 30 verified supersession pairs from the corpus, yielding 91\% precision.
Zero additional API cost is incurred since embeddings are already computed.

\paragraph{LLM confirmation.} For high-similarity pairs ($S \geq 0.82$), we confirm
...
\item \textbf{Parametric memory contamination.}
A critical confound in any closed-domain RAG evaluation is \emph{parametric
leakage}: if the Algerian Official Gazette corpus was included in the pre-training
data of Gemini or Llama-3.3, the models may answer from memorised weights rather
than the retrieved context, artificially inflating Faithfulness and Answer Relevance
scores. We cannot exclude this possibility without white-box access to training data
manifests, which Groq and Google do not disclose. We partially mitigate this by
measuring \emph{Context Precision} and \emph{Context Recall} — metrics that
directly assess whether the retrieved chunks drove the answer — but a fully
controlled study using a held-out, never-published regulatory document set is
necessary to rule out parametric leakage entirely.

\item \textbf{Binary rather than partial supersession.}
The Temporal Supersession Detector [I3] models supersession as a binary
event: a newer document either overrides or does not override an older one.
Algerian regulatory practice frequently involves \emph{partial} supersession,
where a 2024 decree amends only Article 4 of a 2018 circular, leaving the
remaining articles in full force. The current system cannot distinguish this
from total replacement, risking a false alarm that discards still-valid provisions.
Resolving this requires article-level segmentation and cross-document alignment,
which demands a knowledge graph rather than chunk-level cosine similarity.

\item \textbf{Unverified citation hallucination in MALD.}
The Judge agent is instructed to output \texttt{[REF-N]} inline citations, but
no deterministic, programmatic check verifies that the quoted text actually
exists in Chunk~$N$. An LLM can hallucinate a plausible legal clause attributed
to \texttt{[REF-3]} even when that chunk contains no such text. This is the most
dangerous failure mode in legal AI, as it produces confident, cited, but factually
fabricated answers. A post-generation citation verifier — checking each
\texttt{[REF-N]} against the retrieved chunk using fuzzy string matching —
is a necessary addition before production deployment.

\item \textbf{Positional bias and agent sycophancy in MALD.}
LLMs exhibit well-documented \emph{primacy bias}: content appearing first in a
long prompt receives disproportionate attention~\cite{liu2023lost}. The Judge
(Gemini) may systematically favour the Advocate's argument simply because it
is placed first in the prompt, regardless of legal merit. Additionally, both
Advocate and Devil's Advocate run on the same Groq Llama-3.3 model, risking
\emph{sycophantic collusion}: the Devil's Advocate may produce superficial
counter-arguments to avoid generating text that contradicts its twin agent's
conclusion. Mitigations include prompt-order randomisation for the Judge and
using a different model family (e.g., Mistral-7B) for the Devil's Advocate.

\item \textbf{Human evaluation coverage.}
Human annotation covers 50 answers (15\% of the 330-question benchmark), with
Fleiss' $\kappa = 0.61$ inter-rater agreement (substantial). Full 330-question
human annotation with formal Cohen's Kappa reporting is planned for the
camera-ready version. Critically, the system does not yet report a
\emph{False Negative Refusal Rate} — the proportion of valid, answerable
queries that the null-detection gate incorrectly refuses. This metric is as
important as the True Positive Refusal Rate for assessing deployment safety.

\item \textbf{API cost worst-case boundary.}
The reported average cost of \$0.0042/query assumes a single retrieval iteration.
If a complex query triggers the adaptive IQR re-retrieval loop three times, costs
triple to $\approx$\$0.013. The \emph{worst-case cost boundary} across all query
types and iteration counts is not reported, making economic planning for
high-traffic deployments impossible without running the full pipeline on a
cost-distribution study.

\item \textbf{Cross-encoder token truncation.}
The cross-encoder re-ranking step (W5) uses \texttt{mMiniLM-L6} with a strict
512-token context limit. Algerian legal chunks frequently exceed this limit;
the encoder silently truncates the tail of each chunk, potentially discarding
critical legal exceptions or conditions that appear at the end of a regulatory
paragraph. Long-document cross-encoders (e.g., \texttt{jina-reranker-v2-base-multilingual}
with 8,192-token support) would eliminate this blind spot.

\item \textbf{Effective date blindness.}
The temporal weighting system uses document publication year as its proxy for
recency. However, a decree published in 2023 may explicitly state it enters
into force in 2025. The system currently treats the 2023 publication date as
the effective date, potentially surfacing regulations that are not yet legally
operative. Parsing effective-date fields from document headers would provide
legally accurate temporal anchoring.
...
\noindent\textbf{Retrieval \& Embedding Improvements}
\begin{enumerate}[leftmargin=*,nosep]

\item \textbf{Multi-hop knowledge graph retrieval.}
Algerian law relies heavily on referential chains (e.g., \textit{``Decree X amends
Article 4 of Law Y''}). The current system retrieves isolated chunks with no ability
to traverse hierarchical citation graphs. Constructing a knowledge graph over the
corpus — with explicit \textsc{amends}, \textsc{supersedes}, and \textsc{references}
edges — would enable multi-hop legal reasoning that is impossible with flat vector
retrieval.

\item \textbf{Long-document cross-encoder re-ranking.}
The \texttt{mMiniLM-L6} cross-encoder silently truncates chunks beyond 512 tokens,
discarding legal exceptions that appear at the end of regulatory paragraphs.
Replacing it with a long-context reranker (e.g., \texttt{jina-reranker-v2-base-multilingual},
8,192-token limit) would eliminate this blind spot entirely.

\item \textbf{Tuned RRF constant $k$.}
The Reciprocal Rank Fusion constant $k=60$ is borrowed from general IR literature.
A corpus-specific grid search over $k \in \{10, 20, 40, 60, 100\}$ on the 330-query
benchmark would provide empirically grounded fusion weights for the Algerian legal
score distribution.

\item \textbf{Full out-of-domain evaluation.}
Evaluate SPIRAL-RAG on a secondary Arabic legal corpus (Algerian Family Code or ALQAC
dataset) without code changes, to demonstrate that the architecture generalises
beyond the Ministry of Higher Education domain.

\end{enumerate}

\vspace{6pt}
\noindent\textbf{MALD Robustness}
\begin{enumerate}[leftmargin=*,nosep,resume]

\item \textbf{Citation hallucination verifier.}
Add a post-generation verification pass that checks each \texttt{[REF-N]} citation
against the retrieved chunk text using fuzzy string matching (Levenshtein distance
$\leq 0.15$). Any citation whose quoted text cannot be found in Chunk~$N$ is flagged
and suppressed before delivery to the user.

\item \textbf{Prompt-order randomisation and cross-family debate.}
Mitigate primacy bias in the Judge by randomly swapping Advocate/Devil's Advocate
order at inference time. Use a different model family (e.g., Mistral-7B via Groq)
for the Devil's Advocate to prevent sycophantic collusion between agents from the
same model.

\item \textbf{RLHF / DPO alignment of the Judge agent.}
Replace the generalist Gemini Judge with a local Llama-3-8B model fine-tuned via
\emph{Reinforcement Learning from Human Feedback} (RLHF) or \emph{Direct Preference
Optimisation} (DPO)~\cite{rafailov2023dpo}. Legal domain experts compare pairs of
Judge outputs and label preferences; the model is aligned to produce answers that
prioritise citation accuracy, appropriate caution, and Algerian administrative
reasoning style — properties that a generalist cloud model cannot be trusted to
exhibit by default.

\end{enumerate}

\vspace{6pt}
\noindent\textbf{Temporal \& Legal Precision}
\begin{enumerate}[leftmargin=*,nosep,resume]

\item \textbf{Partial supersession detection.}
Extend the Temporal Supersession Detector to distinguish \emph{partial} from
\emph{total} supersession. A 2024 decree may amend only Article 4 of a 2018
circular, leaving all other articles operative. Article-level segmentation plus
cross-document alignment would surface the precise scope of each supersession,
avoiding false discards of still-valid provisions.

\item \textbf{Effective date parsing.}
Parse explicit in-force date fields from document headers (e.g.,
\textit{``entre en vigueur le 1er janvier 2025''}) to replace publication-year
temporal weighting with legally accurate effective-date anchoring.

\end{enumerate}

\vspace{6pt}
\noindent\textbf{Reinforcement Learning (Strategic Direction)}
\begin{enumerate}[leftmargin=*,nosep,resume]

\item \textbf{RL for dynamic intent-weight adaptation.}
Replace the static per-intent weight table (Table~\ref{tab:intent_weights}) with
a Reinforcement Learning agent trained on user feedback signals (thumbs up/down on
retrieved context quality). The RL agent learns to adjust $\mathbf{w}^\phi$ per
query in real time, transcending the limitations of any fixed weight simplex and
providing a principled, data-driven routing mechanism that improves continuously
with deployment usage.

\item \textbf{Strict exact-match metric, full human annotation, and external baseline.}
For the \emph{Temporal} and \emph{Comparative} subsets, report exact-match citation
recall on annotated decree numbers independently of RAGAS. Complete the full
330-question human annotation with Cohen's Kappa. Compare against LlamaIndex and
LangChain out-of-the-box pipelines to establish superiority over industry-standard
frameworks.
...
\textit{NeurIPS}, vol.~36, 2023.

\bibitem{liu2023lost}
N.~F.~Liu et al., ``Lost in the Middle: How Language Models Use Long Contexts,''
\textit{TACL}, vol.~12, pp.~157--173, 2024.
...
%%─────────────────────────────────────────────────────────────────
\subsection{W3 — Lexical Jaccard matching misses vocabulary-divergent supersessions}

\textbf{Concern.} Simple token-set Jaccard similarity used for temporal
supersession detection fails when a newer regulation supersedes an older one
using entirely new terminology. If a 2024 decree completely overhauls a 2018
circular with different vocabulary, the Jaccard score will be low and the
supersession will be missed — a critical failure mode for legal safety.

\textbf{Resolution.} Token-set Jaccard is replaced entirely by
\emph{dense cosine similarity} over the Gemini \texttt{text-embedding-004}
vectors ($\mathbb{R}^{768}$) that are already computed by the
\texttt{DenseRetriever} at query time:
\[
  S(c_i, c_j) = \frac{\mathbf{v}(c_i) \cdot \mathbf{v}(c_j)}
                     {\|\mathbf{v}(c_i)\|\,\|\mathbf{v}(c_j)\|}
\]
Dense embeddings capture \emph{semantic} overlap regardless of surface
vocabulary change — precisely the failure mode that Jaccard cannot handle.
The confirmation threshold is calibrated to $S \geq 0.72$ (low-similarity gate)
and $S \geq 0.82$ (LLM confirmation trigger), validated on 30 verified
supersession pairs from the corpus achieving 91\% precision.
This change incurs \emph{zero additional API cost} since embeddings are
pre-computed during retrieval.

%%─────────────────────────────────────────────────────────────────
\subsection{W4 — 99.1\% LLM Fallback Rate Creates Massive Initialisation Cost}

\textbf{Concern.} The Appendix explicitly states that 8,546 of 8,622 document
titles (99.1\%) fail the regex pattern matching and require an LLM fallback.
This renders the ``regex-first'' approach practically useless and creates a
significant per-session API cost for lazy authority classification.

\textbf{Resolution.} The v3.1 architecture replaces the regex/LLM hybrid with
a \textbf{SetFit zero-shot classifier} (\texttt{sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2},
118\,M parameters) operating on document titles.  SetFit requires no fine-tuning
data: three prototype sentences per tier are sufficient for zero-shot inference:

\begin{lstlisting}[style=pythonstyle,caption={SetFit zero-shot authority classification.}]
TIER_PROTOTYPES = {
    1: ["journal officiel", "loi organique", "decret presidentiel"],
    2: ["decret executif", "arrete ministeriel", "decision interministerielle"],
    3: ["circulaire", "instruction de service", "note de service"],
}
# classify title via cosine similarity to tier prototype embeddings
scores = {t: max(cos_sim(embed(title), embed(p)) for p in protos)
          for t, protos in TIER_PROTOTYPES.items()}
tier = max(scores, key=scores.get)
\end{lstlisting}

This local classifier runs in \textbf{$\sim$2\,ms per title} at zero API cost,
reducing the LLM fallback rate from 99.1\% to an estimated $<$8\% (titles
with similarity scores below a 0.60 confidence threshold, sent to Groq for
confirmation).  Authority classification cost decreases by $\approx 91\%$
per session, and the classifier runs fully offline without API dependency.
...
the debate agents receive context.  Using a multilingual cross-encoder
(e.g., \texttt{cross-encoder/mmarco-mMiniLMv2-L6-H384-v1} or \texttt{BGE-M3},
both supporting Arabic and French natively), each
...
if $\geq 1$ token matches (utilising fuzzy matching via Levenshtein distance
$\leq 2$ to account for non-standardised spelling variations endemic to
written Darija), the language is classified as \texttt{"dz"}
