...
camera-ready version. Current coverage is sufficient for significance testing
on the reported sub-sample results.

\item \textbf{Domain generalisability.}
The current evaluation is limited to Algerian Higher Education regulations
(2018--2024). Reviewers correctly note that architecture claims require
out-of-domain validation. Section~\ref{sec:reviewer-response} (W7) reports
a 20-query adversarial probe set; a full secondary-corpus evaluation on
an open Arabic legal dataset is planned for v4.

\item \textbf{API cost at scale.}
At \$0.0042/query, a system serving 10,000 queries/day costs $\approx$\$42/day in
API fees. The SetFit authority classifier (W4) reduces authority-related LLM calls
by $\approx$91\%, dropping per-query cost to an estimated \$0.0038.
High-traffic deployments should further consider local model hosting
(e.g., Llama-3.3 via vLLM, Gemma-3 for synthesis).

\item \textbf{MALD latency.}
The Advocate + Devil's Advocate calls add $\sim$400--600ms compared to single-pass
synthesis (even in parallel). SSE streaming (W12, \texttt{/api/stream}) eliminates
\emph{perceived} latency by surfacing live reasoning traces during inference.
For throughput-sensitive deployments, MALD can be conditionally disabled for
high-confidence queries (confidence $> 0.85$).

\item \textbf{Corpus immutability assumption.}
Dynamic ingestion (W10) supports incremental chunk-level updates via manifest
checksums; however, re-embedding new chunks still requires Gemini API calls.
A local embedding model (e.g., multilingual E5) would make incremental
ingestion fully offline.
...
\begin{enumerate}[leftmargin=*,nosep]
\item \textbf{Learned intent weights [W1]:} replace the manually-tuned
  $\Delta_4$ simplex weight vectors with Bayesian Optimisation or a lightweight
  MLP trained on a 50-query labelled development set, providing scientific
  grounding for the routing coefficients.
\item \textbf{Full out-of-domain evaluation [W7]:} evaluate SPIRAL-RAG on a
  secondary Arabic legal corpus (e.g., the Algerian Family Code or ALQAC dataset)
  without code changes, to demonstrate architecture generalisability.
\item \textbf{Strict exact-match metric for temporal queries [W8]:} for the
  \emph{Temporal} and \emph{Comparative} benchmark subsets, annotate expected
  decree numbers and years, and measure exact-match citation recall independently
  of RAGAS.
\item \textbf{Full 330-question human annotation [W9]:} complete the 5-expert
  annotation with formal Cohen's Kappa reporting for all benchmark questions.
\item \textbf{Knowledge graph for temporal linking:} link articles across years via
  explicit supersession edges, enabling multi-hop temporal reasoning beyond the
  pairwise detection of v3.1.
\item \textbf{DarijaBERT language ID [W11]:} replace the heuristic Arabic-script
  Darija lexicon with a fine-tuned DarijaBERT or n-gram Naive Bayes classifier
  trained on Algerian social media text.
\item \textbf{Conditional MALD:} skip MALD for queries with confidence $>0.85$
  to reduce cost and latency on unambiguous, high-certainty questions.
